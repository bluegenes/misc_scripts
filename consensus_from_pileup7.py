...
def parseBases(refAllele, baseInf):
	...
	#base.isalpha(), but then might catch the'M' .. maybe should do: isalpha() and not == 'M'
	    ...
	    try: 
	        numBases = int(baseInf[i+1])
	        endIndex = i + 2 + numBases
#	        alleleList.append(line[i:endIndex].upper()) # need to turn bases into uppercase
	        newA = alleleList[-1] + baseInf[i:endIndex].upper() # attach indel to the base before it.
	        alleleList[-1] = newA
		i = endIndex - 1 # advance i by the number of items in the indel
	    except:
		print 'indel issue' # there's at least one line where - appears and isn't followed by a number or any bp ... not correct pileup format = ignore for now.
	i = i + 1
    alleleD ={}
    for b in alleleList:
...
def refToAlt_byContig(contigName, refBases, outAltReference, outSNP, contigDt, depth, refBase):    
	...
	if int(coordInfo[0]) >= depth: #user-specified depth coverage
	    pileupBases = coordInfo[2]
	    newAllele = pileupBases[0]
	    if len(pileupBases) > 1: # there's an indel!
	        if pileupBases[1] =='-':  # indel provision
		    endDeletion = coordinate  + 1 + int(pileupBases[2])
		    deleteIndices.append((coordinate + 1 , endDeletion))
	        elif pileupBases[1] =='+':
		    try:
		        newAllele = newAllele + pileupBases[2:] 
		    except:
		        print 'indel issue?'#just need something here...
		        #import pdb; pdb.set_trace()
		    insertions = True
	    #newBases[coordinate] = newAllele 
	elif refBase:
	    newAllele = refBases[coordinate]
	#PROBLEM HERE WITH PHIL'S DATA
	if coordinate >= len(newBases):
	    print contigName
	    print 'newBases: ' + str(len(newBases)) 
	    print newBases
	    print 'coordinate: ' + str(coordinate)
	    print 'refBases: ' + str(len(refBases)) 
	    print refBases
	newBases[coordinate] = newAllele # why would coordinate be beyond newBases? unless we're not getting appropriate reference sequence? 
...
with open(opts.pileup, 'r') as p:
    with open(opts.reference, 'r') as f:
	    ...
	    if len(line) == 6: # locations with no coverage only have four items in a pileup line
	        ...
	        if len(baseDt) > 0: # deletions will have empty base dictionary --> ignore these lines b/c deletion *already* represented
  		    mostFreqBase, freq = max(baseDt.items(), key=lambda x:x[1])
	            if cName == currentContig: 
	                ...
	                for line in f:
		                ...
		                contigD[int(coordinate)] = [int(coverage), refAllele, mostFreqBase, freq, baseDt] # catch first coordinate
		                break
		            else:    
		                bp = bp + line.strip()
